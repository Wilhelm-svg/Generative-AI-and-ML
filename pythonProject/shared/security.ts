...
export function detectPromptInjection(input: string): { safe: boolean; reason?: string } {
  console.log('[SECURITY] Checking input:', input.substring(0, 100));
  for (const pattern of INJECTION_PATTERNS) {
    if (pattern.test(input)) {
      console.log('[SECURITY] BLOCKED by pattern:', pattern);
      return { safe: false, reason: `Potential prompt injection detected` };
    }
  }
  console.log('[SECURITY] Input passed all checks');
...
export function moderateContent(input: string): { allowed: boolean; reason?: string } {
...
async function logSecurityEvent(
  ...
  try {
    ...
    await db.query(
      `INSERT INTO security_events (project, event_type, user_id, input_hash, reason, severity)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [project, eventType, userId, inputHash, reason, severity]
    );
  } catch {
