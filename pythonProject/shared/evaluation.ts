...
async function llmJudge(prompt: string, apiKey: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`LLM judge failed: ${res.status}`);
...
Respond with JSON only: {"score": 0.0-1.0, "reasoning": "brief explanation"}`;
  ...
  try {
    ...
    const parsed = JSON.parse(raw.match(/\{[\s\S]*\}/)?.[0] ?? "{}") as { score?: number; reasoning?: string };
...
export async function detectHallucination(
  ...
  context: string,
  apiKey: string
): Promise<{ hallucinated: boolean; confidence: number; reasoning: string }> {
...
Respond with JSON only: {"hallucinated": true/false, "confidence": 0.0-1.0, "reasoning": "brief explanation"}`;
  ...
  try {
    const raw = await llmJudge(prompt, apiKey);
    const parsed = JSON.parse(raw.match(/\{[\s\S]*\}/)?.[0] ?? "{}") as { hallucinated?: boolean; confidence?: number; reasoning?: string };
    return {
...
export async function saveEvalResult(
  ...
  query: string,
  response: string,
  result: Partial<RAGEvalResult>,
  queryId?: string
): Promise<void> {
  try {
    const db = await getDbAsync();
    await db.query(
      `INSERT INTO eval_results (project, eval_type, query_id, query, response, groundedness, groundedness_score, answer_relevance, hallucination, judge_reasoning)
       VALUES ($1,$2,$3,$4,$5,$6,$6,$7,$8,$9)`,
      [
        project, evalType, queryId ?? null, query, response,
...
export async function getEvalSummary(project: string) {
  const db = await getDbAsync();
