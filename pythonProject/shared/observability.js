...
export async function logRequest(metrics) {
    const cost = computeCost(metrics.model, metrics.tokensIn, metrics.tokensOut);
    try {
        const db = getDb();
        await db.query(`INSERT INTO request_logs (project, endpoint, user_id, latency_ms, tokens_in, tokens_out, cost_usd, status, error_msg)
...
export async function withObservability(metrics, fn) {
...
export async function getStats(project, hours = 24) {
    const db = getDb();
    const result = await db.query(`SELECT
     ...
     FROM request_logs
     WHERE project = $1 AND created_at > NOW() - INTERVAL '1 hour' * $2`, [project, hours]);
